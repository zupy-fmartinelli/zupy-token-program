...
use crate::constants::{LIGHT_COMPRESSED_TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID, USER_SEED};
...
/// signing with user PDA seeds via `invoke_signed`. The backend builds the complete
...
/// 3. User PDA derivation must match `user_id` (prevents signing for wrong PDA)
...
///   3. user_pda                  (read)             — compressed source + CPI signer
...
pub fn process(
    ...
    // Build account metas from CPI accounts, forcing user_pda to be signer.
    // The user_pda is not a signer on the outer transaction (only our program
    // can sign for it), but it must be a signer in the CPI to the cToken program.
    // invoke_signed provides this signature via the user PDA seeds.
    let mut account_metas = Vec::with_capacity(cpi_accounts.len());
    for acct in cpi_accounts {
        let is_user_pda = acct.address() == user_pda.address();
        let meta = match (acct.is_writable(), acct.is_signer() || is_user_pda) {
    ...
    // ── CPI: Forward V1 TRANSFER to cToken, signing with user PDA ─────────
...
mod tests {
    ...
    #[test]
    fn test_return_user_to_pool_v1_not_enough_account_keys() {
    ...
    #[test]
    fn test_return_user_to_pool_v1_short_data_returns_error() {
        let program_id = Address::default();
